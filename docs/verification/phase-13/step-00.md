...
- Pre-migration `error` rows with default retry checkpoints are adopted as due legacy work, so upgrading does not strand previously quarantined items.
...
- GREEN focused: 43 tests passed across automation config, inventory, runtime, and session storage, including the post-rollout legacy-error compatibility regression.
