...
The first durable traversal is a baseline. Every file observed before its atomic completion is historical and never becomes a DownloadEvent. A versioned continuation yields after 256 entries or 50 ms, so a restart resumes rather than restarts a large tree. Later traversal is the correctness source: a native watcher is only a 250 ms coalesced wake-up hint, while every active root is reconciled at least every 60 seconds. Missing directories and unsupported/failed watchers therefore recover without an event.
