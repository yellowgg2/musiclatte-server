...
`createSessionStore` keeps session/CSRF/capabilities in memory. Absolute expiry clears the account; returning to the visible page revalidates with GET. A transient capability failure preserves the restored account and retries capability discovery after 1, 3 and 10 seconds; only an authenticated 401 expires the session, while a persistent outage retains the explicit retry state. Before an expired session's next password exchange, GET clears an expired HttpOnly cookie or restores the current CSRF. Failed logout retains a retry path; a CSRF rejection refreshes the current cookie context without automatically resending a password. Request generations discard late responses after logout or a newer operation, and a different username/CSRF clears the previous capability snapshot. Disposal invalidates outstanding responses and clears session-expiry/capability-retry timers and subscriptions.
