...
export interface CapabilitiesResponse {
  ...
  instanceId: string;
  revision: string;
  features: Partial<Record<FeatureKey, FeatureCapability>>;
}
...
export const featureCapabilitySchema = {
  type: 'object',
  required: ['supported', 'permission', 'availability'],
  properties: {
...
export const capabilitiesSchema = {
  type: 'object',
  required: ['schemaVersion', 'instanceId', 'revision', 'features'],
  properties: {
    ...
    features: {
      type: 'object',
      required: ['music.browse'],
      properties: Object.fromEntries(featureKeys.map((key) => [key, featureCapabilitySchema])),
    },
  },
} as const;
export const discoverySchema = {
  type: 'object',
  required: ['protocol', 'schemaVersion', 'instanceId', 'apiBase', 'authSchemes'],
  properties: {
    protocol: { const: 'musiclatte-server' },
    schemaVersion: { const: 1 },
    instanceId: string,
    apiBase: { const: '/api/v1' },
    authSchemes: {
      type: 'array',
      minItems: 1,
      uniqueItems: true,
      items: { enum: ['cookie', 'bearer'] },
    },
  },
} as const;
...
export function decodeDiscovery(value: unknown): DiscoveryResponse {
  ...
  if (
    v.protocol !== 'musiclatte-server' ||
    v.schemaVersion !== 1 ||
    v.apiBase !== '/api/v1' ||
    !Array.isArray(v.authSchemes) ||
    !v.authSchemes.length ||
    new Set(v.authSchemes).size !== v.authSchemes.length ||
    !v.authSchemes.every((x) => x === 'cookie' || x === 'bearer')
  )
    throw new Error('Invalid private schema');
  return {
    protocol: 'musiclatte-server',
...
export function discoveryOutcome(
  status: number,
  body: unknown,
): { extension: 'available' | 'absent' | 'unknown'; standard: 'preserve' } {
  if (status === 404 || status === 410) return { extension: 'absent', standard: 'preserve' };
  if (status === 200) {
    try {
      decodeDiscovery(body);
      return { extension: 'available', standard: 'preserve' };
    } catch {
