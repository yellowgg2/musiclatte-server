export type SessionExchange =
  ...
  | { kind: 'subsonic-token'; username: string; t: string; s: string };
...
export const sessionExchangeSchema = {
  oneOf: [
    {
      type: 'object',
    ...
    },
    {
      type: 'object',
      additionalProperties: false,
      required: ['kind', 'username', 't', 's'],
      properties: {
        kind: { const: 'subsonic-token' },
        username,
        t: { type: 'string', pattern: '^[a-f0-9]{32}$' },
...
export const sessionResponseSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['schemaVersion', 'username', 'authScheme', 'expiresAt'],
  properties: {
    schemaVersion: { const: 1 },
    username,
    authScheme: { enum: ['cookie', 'bearer'] },
