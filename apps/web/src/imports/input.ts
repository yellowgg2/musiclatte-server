...
function parseYouTubeSource(input: string): { videoId: string; canonicalUrl: string } {
...
/** Whitespace separates individual links; canonical bodies omit tracking parameters. */
export function parseImportInput(value: string): string[] {
  const lines = value.trim().split(/\s+/);
  if (!value.trim()) throw new Error('invalid_source');
  return lines.map((line) => parseYouTubeSource(line).canonicalUrl);
}
