...
import {
  ...
  galleryCover,
  galleryFixtureId,
  galleryOrganizationStates,
  gallerySwatches,
} from './gallery-fixtures';
import '../design/global.css';
import styles from './Gallery.module.css';

const galleryOrganizationByTrack = new Map(
  galleryOrganizationStates.map((item) => [
    item.target.kind === 'track' ? item.target.trackId : '',
    item,
  ]),
);

const galleryMetadataFetcher: typeof fetch = async (input, init) => {
  const url = new URL(String(input), 'http://gallery.local');
  if (url.pathname.endsWith('/metadata-organization/statuses')) {
    ...
    return Response.json({
      schemaVersion: 1,
      capturedAt: 4,
      items: body.targets.map((target) => galleryOrganizationByTrack.get(target.trackId)),
    });
  }
  const match = url.pathname.match(/\/tracks\/([^/]+)\/metadata$/);
  if (match) {
    const trackId = decodeURIComponent(match[1]!);
    const song = librarySongs.find((entry) => entry.id === trackId)!;
    return Response.json({
      schemaVersion: 1,
      trackId,
      editable: true,
      reason: null,
      format: 'mp3',
      supportedFields: ['title', 'artist', 'album'],
      fileRevision: `gallery-${trackId}`,
      values: {
        title: song.title,
...
export function Gallery() {
  ...
  return (
    <MetadataSyncProvider
      scope="gallery"
      enabled={false}
      fetcher={galleryMetadataFetcher}
      apiOrigin=""
      csrfToken="gallery-csrf"
              ...
              <SongList view="list" aria-label={t['music.songs']}>
                {librarySongs.map((song, position) => (
                  <MusicRow
                    ...
                    playbackStatus={
                      previewPlaying && previewSongId === song.id ? 'playing' : 'paused'
                    }
