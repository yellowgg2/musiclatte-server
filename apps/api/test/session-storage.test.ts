...
describe('session and instance storage', () => {
  ...
  it('should migrate fresh storage and preserve instance across restart', async () => {
    ...
    expect(reopened.connection.prepare('PRAGMA user_version').get()).toEqual({ user_version: 28 });
  ...
  it('should preserve version 25 inventory rows while adding durable retry storage', async () => {
    ...
    expect(upgraded.connection.prepare('PRAGMA user_version').get()).toEqual({ user_version: 28 });
    expect(
