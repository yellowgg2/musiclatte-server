...
describe('session and instance storage', () => {
  ...
  it('should migrate fresh storage and preserve instance across restart', async () => {
    ...
    expect(reopened.connection.prepare('PRAGMA user_version').get()).toEqual({ user_version: 20 });
  ...
  it('should migrate v1 storage to v3 atomically and preserve management data', async () => {
    ...
    expect(migrated.connection.prepare('PRAGMA user_version').get()).toEqual({ user_version: 20 });
    expect(
