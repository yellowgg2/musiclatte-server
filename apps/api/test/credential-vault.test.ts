...
describe('credential vault and persistent key', () => {
  ...
  it('should round trip synthetic proof with distinct ciphertext', async () => {
    ...
    const a = c.vault.seal(proof, 'session-a');
    const b = c.vault.seal(proof, 'session-a');
    expect(a).not.toBe(b);
    expect(a).not.toContain(proof.t);
    expect(a).not.toContain(proof.s);
  ...
  it('should reject tampering malformed versions and wrong keys without raw causes', async () => {
    const c = await makeSUT();
    const envelope = c.vault.seal(proof, 'ctx');
    ...
    try {
      other.open(envelope, 'ctx');
      expect.fail('wrong key accepted');
    } catch (error) {
      expect(error).toMatchObject({ message: 'Reauthentication required' });
  ...
  it('should persist private key files and refuse overwrite', async () => {
    const c = await makeSUT();
    const before = readFileSync(c.keyPath);
    expect(before.length).toBe(32);
    expect(statSync(c.keyPath).mode & 0o777).toBe(0o600);
  ...
  it('should reject missing truncated permissive and symlink keys', async () => {
    const c = await makeSUT();
    const link = join(c.root, 'link.key');
    symlinkSync(c.keyPath, link);
    expect(() => c.loadKey(link)).toThrow('Reauthentication required');
    chmodSync(c.keyPath, 0o644);
    expect(() => c.loadKey(c.keyPath)).toThrow('Reauthentication required');
    chmodSync(c.keyPath, 0o600);
    writeFileSync(c.keyPath, 'broken');
    expect(() => c.loadKey(c.keyPath)).toThrow('Reauthentication required');
    unlinkSync(c.keyPath);
    expect(() => c.loadKey(c.keyPath)).toThrow('Reauthentication required');
