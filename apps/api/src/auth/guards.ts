...
export function credentials(
  ...
  required = true,
): { token: string; scheme: AuthScheme } | undefined {
  ...
  if (cookies.length)
    return { token: cookies[0]!.slice(service.cookieName.length + 1), scheme: 'cookie' };
  if (required) throw new ApiError(401, 'unauthenticated');
  return undefined;
}
export function requiredCredentials(request: FastifyRequest, service: SessionService) {
  return credentials(request, service)!;
}
