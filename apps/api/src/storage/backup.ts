import { closeSync, constants, copyFileSync, fsyncSync, mkdirSync, openSync, rmSync, writeFileSync } from 'node:fs';
...
function verifySnapshot(path: string, key: Uint8Array): void {
  ...
  try {
    validateSchema(db);
    if (db.prepare('PRAGMA quick_check').get()?.quick_check !== 'ok' || db.prepare('PRAGMA foreign_key_check').all().length) throw new Error();
    const vault = createCredentialVault(key);
    const instance = db.prepare('SELECT id,policy_revision,key_id FROM instance WHERE singleton=1').get();
    if (!instance || instance.key_id !== vault.keyId || typeof instance.id !== 'string' || typeof instance.policy_revision !== 'number' || !Number.isSafeInteger(instance.policy_revision) || instance.policy_revision < 1) throw new Error();
    for (const raw of db.prepare('SELECT * FROM sessions').iterate()) {
      ...
      if (row.revoked_at === null) {
        if (!row.encrypted_proof || row.policy_revision !== instance.policy_revision || vault.open(row.encrypted_proof, sessionContext(row)).username !== row.username) throw new Error();
      }
    }
  } finally { db.close(); }
}
...
export async function createBackup(database: ManagementDatabase, keyPath: string, destination: string): Promise<void> {
  let owned = false;
  try {
    if (database.connection.isTransaction) throw new Error();
    const key = loadKey(keyPath);
    validateSchema(database.connection);
    if (database.connection.prepare('SELECT key_id FROM instance WHERE singleton=1').get()?.key_id !== createCredentialVault(key).keyId) throw new Error();
    mkdirSync(destination, { mode: 0o700 }); owned = true;
    ...
    verifySnapshot(path, key); syncFile(path); syncFile(join(destination, 'credential.key')); syncFile(destination);
  } catch { if (owned) rmSync(destination, { recursive: true, force: true }); throw new Error('Backup failed'); }
}
...
export async function restoreBackup(source: string, destination: string): Promise<void> {
  ...
  try {
    ...
    try { await backup(snapshot, path); } finally { snapshot.close(); }
    copyFileSync(join(source, 'credential.key'), join(destination, 'credential.key'), constants.COPYFILE_EXCL);
    verifySnapshot(path, loadKey(join(destination, 'credential.key')));
    syncFile(path); syncFile(join(destination, 'credential.key')); syncFile(destination);
  } catch { if (owned) rmSync(destination, { recursive: true, force: true }); throw new Error('Restore failed'); }
}
