...
export function registerScanRoute(app: FastifyInstance, service: SessionService) {
  app.post('/api/v1/scan', { schema: { body: { type: 'object', properties: {}, additionalProperties: false } } }, async request => {
    const auth = requiredCredentials(request, service); requireJSON(request);
    ...
    if (!service.options.allowScan || identity.adminRole !== true) throw new ApiError(403, 'forbidden');
    try { await upstream.startScan(); return { schemaVersion: 1, accepted: true }; }
    catch (error) { return service.rejectUpstream(error, session.raw); }
  });
}
