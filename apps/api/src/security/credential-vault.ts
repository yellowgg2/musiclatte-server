...
export function createCredentialVault(input: Uint8Array): CredentialVault {
  ...
  const aad = (context: string) =>
    Buffer.from(JSON.stringify(['musiclatte-credential', 1, keyId, context]));
  return {
    keyId,
    seal(proof, context) {
      const payload = JSON.stringify(projectProof(proof));
    ...
    open(envelope, context) {
      try {
        ...
        const [version, id, ivText, bodyText, tagText] = parts;
        if (
          parts.length !== 5 ||
          version !== '1' ||
          id !== keyId ||
          ...
          throw new Error();
        const values = [ivText, bodyText, tagText].map((value) => {
          const decoded = Buffer.from(value, 'base64url');
          if (decoded.toString('base64url') !== value) throw new Error();
          return decoded;
        });
        const [iv, body, tag] = values;
        if (!iv || iv.length !== 12 || !body || !tag || tag.length !== 16) throw new Error();
