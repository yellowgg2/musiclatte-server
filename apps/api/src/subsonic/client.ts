...
export interface RandomOptions extends RequestOptions {
  size?: number;
  musicFolderId?: string;
  genre?: string;
  fromYear?: number;
...
export function createSubsonicClient(options: SubsonicClientOptions): SubsonicClient {
  ...
  try {
    ...
    if (
      ...
      throw new Error();
    if (
      typeof options.proof.username !== 'string' ||
      !options.proof.username ||
      typeof options.proof.s !== 'string' ||
      !options.proof.s ||
      !/^[a-f0-9]{32}$/.test(options.proof.t)
    )
      throw new Error();
    origin = url.origin;
  } catch {
    throw new SubsonicError('invalid_configuration');
  }
  ...
  async function request(
    ...
    try {
      ...
      const response = await fetch(buildURL(operation, pairs), {
        method: 'GET',
        redirect: 'manual',
      ...
      const error = external?.aborted
        ? new SubsonicError('cancelled')
        : timedOut
          ? new SubsonicError('timeout')
          : cause instanceof SubsonicError
            ? cause
            : new SubsonicError('network');
  ...
  return {
    ...
    async artist(id, opts) {
      return decodeArtist((await request('getArtist', [['id', required(id)]], opts)).artist);
    },
    async album(id, opts) {
      return decodeAlbum((await request('getAlbum', [['id', required(id)]], opts)).album);
    },
    async random(opts = {}) {
      const pairs = folderPair(opts.musicFolderId);
      for (const key of ['size', 'fromYear', 'toYear'] as const)
        if (opts[key] !== undefined) pairs.push([key, numeric(opts[key])]);
    ...
    mediaRequest(kind, id, opts = {}) {
      if (
        !['stream', 'getCoverArt'].includes(kind) ||
        !['GET', 'HEAD'].includes(opts.method ?? 'GET')
      )
