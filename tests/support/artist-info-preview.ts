/** Synthetic artist enrichment through the real API and normal product route. */
...
const upstream = createServer((request, response) => {
  ...
  const error = !valid
    ...
    : isLibrary && (mode === 'missing' || id === 'missing')
      ...
      : isLibrary && mode === 'error'
        ...
        : undefined;
  if (
    (operation === 'getArtistInfo2' && mode === 'info-error') ||
    (isLibrary && (mode === 'error' || (isRandom && mode === 'random-error')))
  ) {
  ...
  const requestedArtist =
    id === 'artist-2'
      ? { id: 'artist-2', name: 'Moonlight', album: [album] }
      : { id: 'artist-1', name: 'Daylight', album: empty ? [] : [album] };
  const artistInfo =
    mode === 'info-empty'
      ? {}
      : {
          biography:
            '<img src=\"https://listener:secret@example.invalid/x\">' +
            'Server biography. '.repeat(35),
          musicBrainzId: 'fixture-mbid',
          smallImageUrl: 'https://listener:secret@example.invalid/image',
          similarArtist: [{ id: 'artist-2', name: 'Moonlight' }],
        };
  const payload: Record<string, object> = {
    ...
    search3: {
      searchResult3: {
        song: songs,
        artist: empty ? [] : [requestedArtist],
        album: empty ? [] : [album],
      },
    },
    getArtist: { artist: requestedArtist },
    getArtistInfo2: { artistInfo2: artistInfo },
    getAlbum: { album },
...
</head><body><iframe title="Musiclatte ${width}px player preview" src="http://127.0.0.1:${webPort}/music/artists/artist-1"></iframe></body></html>`;
...
await context.app.listen({ host: '127.0.0.1', port: apiPort });
console.info(`S12 artist preview ready at 127.0.0.1:${apiPort}`);
