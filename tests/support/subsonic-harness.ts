...
export async function loadProtocol(): Promise<
  typeof import('../../apps/api/src/subsonic/protocol.js')
> {
...
export async function createTestContext(
  ...
  const { createFakeSubsonic }: typeof import('../../packages/test-support/src/fake-subsonic.js') =
    await import(path);
  const upstream = await createFakeSubsonic(scenario);
  try {
    const client = module.createSubsonicClient({
...
export const ok = (payload: Record<string, unknown> = {}) => ({
  'subsonic-response': { status: 'ok', version: '1.15.0', ...payload },
});
